...
const onReceiveBufferImpl = module.exports = function(b, d, q /* locals: */, pkt, load, key, peer, crc, m, l, ms, msg){
	...
	for(;;){
		...
		if(!m){
			console.log("UdpService::Read: skip-command: iface: %s, unsupported command, %s, code: %s%s", 
				this, 
				pkt, 
				b[0], 
		...
		if(false /* && !peer && this.resolveClientAsync */){
			/**
			console.log("UdpService::Read: resolve-client: iface: %s, peerKey: %s, %s <- @ %s:%s",
				this, 
		...
		if(!peer){
			console.log("UdpService::Read: client-unknown: iface: %s, peerKey: %s, %s : sign: %s, addr: %s:%s", 
				this, 
				FN_FORMAT_BINARY_AS_HEX(key), 
				m.toString(), 
		...
		if(!peer.secret){
			console.log("UdpService::Read: skip-secret: iface: %s, peer: %s, secret is not set, addr: %s:%s", 
				this, 
				peer.shortHostString ?? peer,
				pkt.sourceAddress.address.hostAddress, 
				pkt.sourceAddress.port
		...
		if(load.slice(0, 16) != crc){
			console.log("UdpService::Read: crc-fail: iface: %s, peer: %s, crc mismatch, %s : %s != %s",
				this, 
				peer.shortHostString ?? peer,
				m.toString(), 
				Format.jsObject(load.slice(0, 16)), 
		...
		if(msg){
			if(msg.isReply /** m.prototype.isRequest */){
				...
				console.log("UdpService::Read: send-repeat: iface: %s, peer: %s, reply re-sent by peer, serial: %s, addr: %s:%s", 
					this, 
					peer.shortHostString ?? peer,
					ms, 
					pkt.sourceAddress.address.hostAddress, 
		...
		if(!msg){
			console.log("UdpService::Read: bad-body: iface: %s, peer: %s, invalid payload rejected, %s, serial: %s, packetLen: %s", 
				this, 
				peer.shortHostString ?? peer,
				m.toString(), 
				ms, 
		...
		/*
		console.log("UdpService::Read: received: iface: %s, peer: %s @ %s:%s, len: %s, serial: %s, %s"",
			this, 
			peer.shortHostString ?? peer,
			pkt.sourceAddress.address.hostAddress, 
			pkt.sourceAddress.port,
