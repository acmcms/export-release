window.Tree || (Tree = parent.Tree) || (Tree = function(target, action, highlight){
	// v0.2
	window.top.debug && window.top.debug("Tree: initialize: target=" + target + ", targetNodeName=" + target.nodeName + ", action=" + action);
	...
	router.register(this, 'form');
	window.top.debug && window.top.debug("Tree: document ready, initiating xml query");
	...
	this.element.onclick = function(e){
		...
		window.top.debug && window.top.debug("Tree: onclick event");
	...
	this.element.ondblclick = function(){
		window.top.debug && window.top.debug("Tree: ondblclick event");
...
Tree.prototype = {
	...
	build : function(xmlData, node){
		...
		for (var i = 0; i < xmlData.childNodes.length; ++i){
			...
			oTD.childs.hasChild = currentNode.firstElementChild ? "true" : currentNode.getAttribute("children");
			if( currentNode.nextElementSibling || oTable.nextSibling && oTable.nextSibling.nextElementSibling ){
	...
	selectOn : function(obj){
		with(this){
			...
			setTimeout(function(){
				...
				var cy = Standard.calculateObjectCoordinates(element.currentSelect).y 
									- Standard.calculateObjectCoordinates(element).y;
