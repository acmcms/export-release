...
function EditImage(){
	...
	if(!result) {obj.SetFocus();return;}
	if(result['html']) insertHtml(result['html'],obj);
}
...
function EditTable(item){
	...
	if (obj.DOM.selection.type != 'Text'){
		if (sel.length == 1){
			if (sel.item(0).tagName == "TABLE") table = sel.item(0);
		}
	}
	...
	if(!result){obj.SetFocus(); return;}
	if(result['resultType'] == 'modify'){obj.SetFocus(); return;}
...
function EditLink(){
	...
	obj.ExecCommand(DECMD_HYPERLINK, OLECMDEXECOPT_DONTPROMPTUSER, "javascript:void(0);/*fckeditortemplink*/");

	for (i = 0 ; i < obj.DOM.links.length ; ++i){
		if ( obj.DOM.links[i].href == "javascript:void(0);/*fckeditortemplink*/" ){
			obj.DOM.links[i].href = result['path'];
			if (result['title'] == "") obj.DOM.links[i].removeAttribute("title",0) ;
			else obj.DOM.links[i].title  = result['title'];
			if (result['target'] == null || result['target'] == '') obj.DOM.links[i].removeAttribute("target",0) ;
			else obj.DOM.links[i].target = result['target'];
		}
	}
