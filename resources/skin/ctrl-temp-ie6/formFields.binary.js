...
function setImage(objID,fieldName){
	var result = showModalDialog( "form-binary-upload-modal.htm?id=<%= Request.id %>&data="+fieldName,false,"dialogWidth:500px; dialogHeight:145px; scroll:no; status:no; resizable:yes; help:no");
	if(!result) return;
	var iFields = Dom.getElementsByAttributeName(document.getElementById(objID), 'info');
